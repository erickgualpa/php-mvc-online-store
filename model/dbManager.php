 ...
 * Se comprueba que todos los inputs del formulario hayan sido introducidos correctamente
 * en cuyo caso se retorna 'true' para iniciar la llamada a la función de registro en BBDD.
 */
    ...
    function validateRegister(){
            ...
            }else{echo"<script>alert('El nombre de usuario no es correcto.');</script>";
                $validUser = false;}

            // Validate user 'email'
            if(filter_var($email, FILTER_VALIDATE_EMAIL)){
                // echo "el email es correcto"
            }else{echo"<script>alert('El email no es correcto.');</script>";
                $validUser = false;}

            // Validate user 'password'
            if(ctype_alnum ((string) $password )){
                // echo "la contraseña es correcta"
            }else{echo"<script>alert('La contraseña no es correcta');</script>";
                $validUser = false;}

            // Validate user 'address'
            if((strlen((string) $address) < 31) &&  preg_match('/^[A-Za-z0-9- ]+$/', (string)$address)){
                // echo "La dirección es correcta"
            }else{echo"<script>alert('La dirección no es correcta');</script>";
                $validUser = false;}

            // Validate user 'town'
            if((strlen((string) $town) < 31) && ctype_alnum ((string) $town )){
                // echo "La ciudad es correcta"
            }else{echo"<script>alert('La ciudad no es correcta');</script>";
                $validUser = false;}
            ...
            }else{echo"<script>alert('El código postal no es correcto');</script>";
            $validUser = false;}
